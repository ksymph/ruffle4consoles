...
/// Returns (free_user, free_cdram, free_phycont) in bytes, or None on error.
#[cfg(target_os = "vita")]
pub fn get_vita_memory() -> Option<(usize, usize, usize)> {
    unsafe {
...
/// vglMemType enum values matching vitaGL.h
#[cfg(target_os = "vita")]
const VGL_MEM_VRAM: u32 = 0;
#[cfg(target_os = "vita")]
const VGL_MEM_RAM: u32 = 1;
#[cfg(target_os = "vita")]
const VGL_MEM_SLOW: u32 = 2; // PHYCONT_USER_RW
#[cfg(target_os = "vita")]
const VGL_MEM_BUDGET: u32 = 3;
#[cfg(target_os = "vita")]
const VGL_MEM_EXTERNAL: u32 = 4;
#[cfg(target_os = "vita")]
const VGL_MEM_ALL: u32 = 5;

#[cfg(target_os = "vita")]
unsafe extern "C" {
    fn vglMemTotal(mem_type: u32) -> usize;
    fn vglMemFree(mem_type: u32) -> usize;
}

/// Query VitaGL internal pool memory. Returns (total, free) for each pool.
/// Returns None on non-Vita platforms.
#[cfg(target_os = "vita")]
pub fn get_vgl_pool_stats() -> Option<(
    (usize, usize), // VRAM (CDRAM)
    (usize, usize), // RAM (USER_RW)
    (usize, usize), // SLOW (phycont)
    (usize, usize), // ALL
)> {
    unsafe {
        Some((
            ...
            (vglMemTotal(VGL_MEM_ALL), vglMemFree(VGL_MEM_ALL)),
        ))
    }
}
...
pub fn log_heartbeat(state: &DiagnosticsState) {
    ...
    let vgl_str = match get_vgl_pool_stats() {
        Some((vram, _, slow, _)) => {
            format!(
                " | VGL: vram={}/{} slow={}/{}",
                format_bytes(vram.1),
...
pub fn log_memory_snapshot(label: &str) {
    ...
    match get_vita_memory() {
        Some((user, cdram, phycont)) => {
            parts.push(format!("user={}", format_bytes(user)));
            parts.push(format!("cdram={}", format_bytes(cdram)));
            parts.push(format!("phycont={}", format_bytes(phycont)));
        }
        None => {
            parts.push("sceKernelGetFreeMemorySize: N/A".to_string());
        }
    }
    match get_vgl_pool_stats() {
        Some((vram, ram, slow, total)) => {
            parts.push(format!(
                "VGL: vram={}/{} ram={}/{} slow={}/{} total={}/{}",
                format_bytes(vram.1), format_bytes(vram.0),
                format_bytes(ram.1), format_bytes(ram.0),
                format_bytes(slow.1), format_bytes(slow.0),
                format_bytes(total.1), format_bytes(total.0),
            ));
        }
        None => {}
    }
    tracing::info!("[MEMORY @ {}] {}", label, parts.join(" | "));
}
